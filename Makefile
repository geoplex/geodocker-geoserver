...
GEOMESA_PROCESS_JAR := geomesa-process-${GEOMESA_VERSION}.jar

download-plugins:
	mkdir -p .cache \
&& cd .cache \
&& curl -L -C - -O "http://repo.locationtech.org/content/repositories/geomesa-releases/org/locationtech/geomesa/geomesa-process/${GEOMESA_VERSION}/${GEOMESA_PROCESS_JAR}" \
&& cd .. && cp .cache/${GEOMESA_PROCESS_JAR} ${GEOMESA_PROCESS_JAR}

.cache/${GEOMESA_DIST_TARBALL}: download-plugins
	cd .cache \
